...
require_once("modules/configuracion/clases/configuracion_municipios_view.php");

class configuracion_municipios_controller{
    
    private $objView;
    private $strDepto;
    public function __construct(){
        $this->objModel = new configuracion_municipios_model();
        $this->objView = new configuracion_municipios_view();
    }
    
    public function setDepto(){
        $strDepto = isset($_GET["municipio"]) ? $_GET["municipio"] : "";
    ...
    public function fntPost(){
        ...
        if( isset($_POST['hdnDepartamento']) && isset($_POST["hdnEliminar"]) && $_POST["hdnEliminar"] == "N" ){
            ...
            if( $boolInsert  ){
                    ...
                    ?>
                    <script>
                        document.location = "<?php echo $strAction."?municipio=".(md5($intDepto))."&strAlert=ins"; ?>";
                    </script>
                    <?php
                    //die();
                }
